...
cout << endl;
//An unordered map indexed by the type with a lambda expression "value"
unordered_map<type_index, std::function<string()>> type_func;
type_func[typeid(Sender)] = []() { return "Send something"; };
type_func[typeid(Receiver)] = []() { return "Receive something"; };
for (auto itr = type_func.begin(); itr != type_func.end(); ++itr)
{
//*itr is a std::pair
cout << (*itr).first.name() << ' ' << (*itr).second() << endl;
}
//cout << type_func[typeid(int)]() << endl; //error!!
