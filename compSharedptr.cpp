...
cout<<*sp1<<endl<<endl; //2. Creating shared pointer from another shared pointer
auto sp2( sp1);
auto sp3 = sp2; //3. Creating a shared pointer from a raw pointer
auto sp4 = shared_ptr<int>( new int(100) ); //4. Creating shared pointer with a nullptr
...
// delete pRaw; cout<< *sp1<<endl;
//
// cout << "After pRaw"<<*sp1;
//6. Get how many shared pointers sharing the resource
int nbReferences = sp1.use_count();
cout<<" Number of shared pointers sharing the resource = "<<nbReferences<<endl<<endl; //7. Is this only shared pointer sharing the resource
cout<<"Is sp1 is the only sharing pointer sharing the resource = "<< (sp1.unique() ? "Yes" : "No")<<endl<<endl;
cout<<"Is sp4 is the only sharing pointer sharing the resource = "<< (sp4.unique() ? "Yes" : "No")<<endl<<endl; //8. Dont try to have more than shared pointer from a raw pointer. This cause
//the twice the deletion of the resource and hence acesss violation
int* p = new int(200);
auto sp1FromRaw = shared_ptr<int>(p);
//Uncomment the below lines to get the access violation. auto sp2FromRaw = shared_ptr<int>(p); //Solution of above
// auto sp2FromRaw = sp1FromRaw;
// auto sp3FromRaw = sp2FromRaw; //Notice to get the reference count of either sp1FromRaw or sp2FromRaw. It will give the
...
cout<<" Reference count for raw pointer p = "<<nbReferences<<endl<<endl; //9.Using custom deallocator for creating the shared pointer. The deallocator is a
...
//auto sp15=dynamin_pointer_cast<Derived>(sp14);
if( sp15 )
{​​
